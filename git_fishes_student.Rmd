...
[My GitHub Repository](https://github.com/Ginny370/WEEK6_hm.git)
...
# Write your answer here
#If we change echo = TRUE to echo = FALSE, it means that we want to have code in our rmarkdown file. If echo = FALSE, we can not see code in rmarkdown.
```
...
```{r, library some package, echo=TRUE, eval = FALSE}
library(tidyverse)
library(spelling)
library(plotrix)
```
...
```{r, prettyfig, fig.align='center', fig.cap="boxplot of each species", echo=TRUE, eval = FALSE}
...
```{r, echo=TRUE, eval = FALSE}
## Write your answer here
#I find that the `.gitignore` text file, which contains the path of the boxplot picture, is added into my *GitHub Desktop* and the figure disappear in my output/figure folder on my Github website. 
```
...
#No, because when I need to check each png file, I need to open each of them separately. It will become more tired to check each of them.
```
...
```{r, table1, tab.cap = "t.test of each species"}
...
**Write your answer here**
I would like to choose lemon fish.
In this test, our hypothesis is that there is no significant difference in mean between control and treatment group in lemon fish.
The p-value of lemon is 0.071 in Table \@ref(tab:table1), and the t-statistic is -1.86.
The p-value is bigger than 0.05, which means that there is no significant difference of mean between control and treated group. And small t-statistic with 42 df also indicate that mean of control and treatment has no significant differece.
...
When we permutate our data, we find that there is no significant difference between original t-test and permutated t-test in p-value Tibble \@ref(tib:tibble1). So we can safely arrive at a conclusion that there is no significant difference of mean between control and treantment in species lemon.
