...
```{r some problem with function "flextable"}
...
```{r, don't know the change}
## Write your answer here
#I find that the `.gitignore` text file, which contains the path of the boxplot picture, is added into my *GitHub Desktop*.  
```
...
ggplot(data3, aes(x = treatment, y = activity, color = treatment)) + geom_boxplot() + labs(title = "collab_Fig")  
```
