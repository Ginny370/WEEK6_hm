...
In this test, our hypothesis is that there is no significant difference between control and treatment group in lemon fish.
The p-value of lemon is 0.071 in Table \@ref(tab:table1), and the t-statistic is -1.86.
The p-value is bigger than 0.05, which means that there is no significant difference between control and treated group. And small t-statistic with 42 df also indicate that no significant differece.
...
When we permutate our data, we find that there is no significant difference between original t-test and permutated t-test in p-value Tibble \@ref(tib:tibble1). So we can safely arrive at a conclusion that there is no significant difference between treantment in lemon.
